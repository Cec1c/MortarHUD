...
> **2026-09-22 更新**：M 已改为短时连拍后识别，相邻两帧一致才锁炮位；截完后只允许鼠标移动，其他输入继续取消。诊断整屏和 ROI 同帧、后台保存。Auto 为 C / C-Balanced × PSM 6 / 11，保留全部有效读数一致的要求。新增「炮口标尺」设置页、F10 标尺开关和按客户区分辨率保存的校准；277 项测试及 54 张新 ROI 回归。下面的旧问题记录保留供追溯，当前实现和验收以 [本次验收说明](docs/2026-09-22-acceptance.md) 为准。新产物在 `dist/MortarHUD-20260922-M-ruler-portable/`。

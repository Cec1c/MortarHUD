...
识别失败时 HUD 会说明原因并保留上一个目标，不会拿一个可能是错的坐标继续用。采不到数据也会说清楚——「地图没打开」「光标移动，已取消」——而不是默默什么都不做。
...
当前覆盖 `x y . 0 1 4 5 6 7 8 9`，**缺数字 2 和 3**（基准集里那三张截图的坐标没覆盖到）。遇到不认识的字形输出 `?` 而不是猜值，解析器随即判失败。Tesseract 覆盖全部十个数字，所以生产默认走它。
...
```bash
dotnet test MortarHUD.sln       # 244 个（218 Core + 26 OCR）
```
...
输出到 `dist\MortarHUD-next-<模式>\`，目录非空时拒绝发布（避免新旧 DLL 混在一起）。便携版是**真正的单文件**——`Models\`（OCR 语言包与字形库）也一并打包，运行时释放到 `%TEMP%\.net\`。体积大头是 .NET 运行时和 `OpenCvSharpExtern.dll`（59 MB）；项目自身四个 DLL 加起来不到 0.5 MB。压到 95 MB 靠的是单文件压缩、剔掉 OpenCV 的 FFmpeg 插件和 Tesseract 的 x86 库。
...
其它：模板引擎缺数字 2、3（只在语言包缺失时才会用到它）；没有安装包；ROI 默认值按 1080p 实测，游戏 UI 缩放差得多的话需要用「诊断 → 查看单次识别结果」调一次。

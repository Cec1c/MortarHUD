...
2. **灰阶层次压平**。背景 `#151B21`、卡片 `#202932`、输入框也是 `#202932`
（TextBox 的 `Background` 与 Card 用的是同一个 `SurfaceAltBrush` 键）——卡片与输入框完全没有区分，
   卡片浮不起来，整页看着是一块平板。
...
实现完成后的复盘。以下是与本规格的**有意偏离**，均已落地：

**配色**

- `SubtleTextBrush` 从规格的 `#6B7C8A` 改成 `#7E8F9D`。原值在 12px 正文下对本项目的深色表面只有
  ≈3.7:1，低于 WCAG AA 的 4.5:1；新值在三层表面上分别是 4.73 / 5.26 / 5.00:1，同时仍明显暗于
  `MutedTextBrush`（7.4:1），三级文字层次保住了。
- `Hint` 的前景色从 `MutedTextBrush` 改成 `SubtleTextBrush`（规格的字号表写的是前者）。
- 新增两个键：`AccentHoverBrush` `#93E0CB`（主按钮悬停）、`SurfaceHoverBrush` `#222D35`
  （卡片内控件的悬停底色）。后者的必要性：规格把 `SurfaceCardBrush` 同时当作卡片底色与按钮悬停色，
  结果是卡片里的按钮一悬停就变成和卡片同色——同一语义两套用途，正是本规格诊断 `SurfaceAltBrush` 时
  批评过的毛病。
- 滑块**未填充**轨道从 `SurfaceSunkenBrush` 改成 `BorderStrongBrush`。`#0F151A` 对页面背景
  `#141A1F` 只有 1.06:1 的对比度，滑块看起来是一条绿杠加一个白点、完全看不出量程。

**规格漏掉的控件**

- `ScrollBar`：规格的控件清单里没有它。设置页右侧因此一直是一条 WPF 默认的浅灰滚动条，
  在深色界面上是一道刺眼的白条。补了垂直与水平两套模板。
- `ToolTip`：同样漏了。11 处用到它（滑块值、语言包路径），默认是浅色的 `SystemColors.Info` 框，
  悬停滑块就会弹出一块亮斑。补了深色模板。

**未按规格执行的一处**

- `SectionHeader` 的 `Margin` **保持 `0,16,0,10`**，没有按表格改成 `0,0,0,12`。这个样式同时用在
  卡片内与页面级区块：卡片内靠它撑出与卡片顶边的距离，页面级靠它分隔区块。上边距归零会让页面级
  区块标题紧贴上一行说明文字。

**已知未收口**

- `Views/ColorEditor.xaml`（外观页折叠项里的颜色编辑器）仍是旧视觉语言，自己写了
  `#3A3D42` / `#26282C` 与圆角 3。它在规格的控件清单之外，且只在展开折叠项后才可见。
- 「间距只用 `4 / 8 / 12 / 16 / 22 / 26 / 32`」这条约束**本规格自己就没遵守**：表格里同时给出了
  10（TextBox 内边距、复选框标签间距）、14（导航项与折叠项内边距）、24（区块间距）等值。
  实施时定的口径是「约束只作用于新写的值，不追溯基线的历史值」。这条规则若日后要当真，
  应当先重写它本身。
